...
const Goals = () => {
                    ...
                    const fetchPromises = response.data.data.map(
                         async (pet) => {
                              const [goalsResponse, activitiesResponse] =
                                   await Promise.all([
                                        axios.get(
                                             `http://localhost:3000/api/v1/goals/all-goals/${pet._id}`,
                                             { withCredentials: true }
                                        ),
                                        axios.get(
                                             `http://localhost:3000/api/v1/activities/get-all-activities/${pet._id}`,
                                             { withCredentials: true }
                                        ),
                                   ]);
     ...
     const handleStatusChange = async (goalId, currentStatus) => {
          try {
               ...
               const response = await axios.patch(
                    `http://localhost:3000/api/v1/goals/update-status/${goalId}`,
                    { status: newStatus },
                    { withCredentials: true }
               );
     ...
     const handleDeleteGoal = async (goalId, petId) => {
          try {
               await axios.delete(
                    `http://localhost:3000/api/v1/goals/delete-goal/${goalId}`,
                    { withCredentials: true }
               );
